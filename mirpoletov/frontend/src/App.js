...
function App() {
  ...
  const showErrorWindow = (message) => {
    ...
    setModalWindowTitle("Предупреждение");
    setModalWindowStyle("error");
    setIsModalWindowOpen(true);
  };

  const showLoadingWindow = (message) => {
    setModalWindowText(message);
    setModalWindowTitle("Загрузка");
    setModalWindowStyle("loading");
    setIsModalWindowOpen(true);
  };
  ...
  const addToEnd = (newRegion) => {
    setRegions(prevRegions => [...prevRegions, newRegion]);
  };
  const removeByValue = (value) => {
    setRegions(prevRegions => prevRegions.filter(item => item !== value));
  };
  const clearAll = () => {
    setRegions([]);
  ...
  const addAll = () => {
    ...
    document.querySelectorAll(".region").forEach(region => {
      ...
      addToEnd(region.getAttribute("data-code"));
    });
  };
  ...
  const handleSearchSelect = (regionId) => {
    ...
    if (region) {
      if (!regions.find(s => s === regionId)) {
        addToEnd(regionId);
        document.querySelector(`[data-code="${regionId}"]`).classList.add("active");
        document.title = region.name;
      }
      else {
        removeByValue(regionId);
        document.querySelector(`[data-code="${regionId}"]`).classList.remove("active");
        document.title = "Мир полетов";
      }
    }
  };
  ...
  const HandleCalculate = async () => {
    ...
    const date_check1 = new Date(`${month1}.${day1}.${year1} ${hour1}:${minute1}`);
    const date_check2 = new Date(`${month2}.${day2}.${year2} ${hour2}:${minute2}`); 
    ...
    showLoadingWindow(`Ваши данные были отправлены на сервер. В ближайшее время по ним будут рассчитаны метрики, ожидайте.`);
    try {
  ...
  return (
    <div className={`App ${isHeaderVisible ? "App-header-on" : "App-header-off"}`}>
      ...
      <button className="header-pos-change-button" onClick={HandleHeaderPositionChange}>
